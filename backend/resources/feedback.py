...
class FeedbackList(Resource):
    @classmethod
    def post(cls, application_hash):
        data = request.get_json()
        if not data:
            return {'message': 'No input data provided'}, 400
        feedback = FeedbackModel(hash=application_hash, description=data["description"], user_agent=request.headers.get('User-Agent'), ip=request.remote_addr)
        feedback.save_to_db()
...
class FeedbackAdd(Resource):
    ...
    def post(cls, application_hash):
        ...
        if not data:
            return {'message': 'No input data provided'}, 400
        application = ApplicationModel.find_by_hash(application_hash)
        if not application:
